//Код до застосування методу рефакторингу «Метод перейменування»
...
class FollowersActivity : AppCompatActivity(), OnFollowStateChangedListener {
    ...
    private fun followings(targetUserId: String) {
        val currentUserId = encryptedPreferencesManager.getData("user_id")
        val finalUserId = if (targetUserId.isNotEmpty() && targetUserId != currentUserId) targetUserId else currentUserId

        RetrofitClient.getInstance(applicationContext).userAPI.getFollowings(finalUserId)
    ...
    private fun followers(targetUserId: String) {
        val currentUserId = encryptedPreferencesManager.getData("user_id")
        val finalUserId = if (targetUserId.isNotEmpty() && targetUserId != currentUserId) targetUserId else currentUserId

        RetrofitClient.getInstance(applicationContext).userAPI.getFollowers(finalUserId)
            .enqueue(object : Callback<List<Follower>> {
                override fun onResponse(call: Call<List<Follower>>, response: Response<List<Follower>>) {
                    if (response.isSuccessful) {
                        val data = response.body()
    ...
    private fun show1(followings: List<Follower>) {
    ...
    private fun show2(followers: List<Follower>) {
    ...
    private fun state(id: String, followerId: String, callback: (Boolean) -> Unit) {
...
//Код після застосування методу рефакторингу «Метод перейменування»
...
class FollowersActivity : AppCompatActivity(), OnFollowStateChangedListener {
    ...
    private fun getFollowings(targetUserId: String) {
        val currentUserId = encryptedPreferencesManager.getData("user_id")
        val finalUserId = if (targetUserId.isNotEmpty() && targetUserId != currentUserId) targetUserId else currentUserId
    ...
    private fun getFollowers(targetUserId: String) {
        val currentUserId = encryptedPreferencesManager.getData("user_id")
        val finalUserId = if (targetUserId.isNotEmpty() && targetUserId != currentUserId) targetUserId else currentUserId
