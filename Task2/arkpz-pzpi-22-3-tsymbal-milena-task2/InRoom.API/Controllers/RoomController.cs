...
public class RoomController : ControllerBase
{
    ...
    {
        var room = await _roomService.Add(request.Name, request.ZoneName);
        return Ok(room);
    }
    ...
    [SwaggerOperation("Edit room by id")]
    public async Task<IActionResult> EditRoom([FromRoute] Guid roomId, UpdateRoomRequest request)
    {
        var room = await _roomService.Update(roomId, request.Name, request.ZoneName);
        return Ok(room);
    }
